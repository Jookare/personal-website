...
const buttonTheme = {
    base: "flex rounded-lg items-center text-zinc-400 transition-all duration-200",
    hover: "hover:underline hover:text-zinc-200",
    icon: "sm:h-6 h-4 mr-2"
};

// use inline icons for better color control
...
export const Links = () => {
    ...
    return (
        <div id="links" className="h-12 max-w-[650px] w-full sm:pl-[180px] sm:justify-start justify-center gap-6 flex my-2 z-30 items-center">
            ...
            <SVGButton path={resume} name="Resume" link={"https://lut-my.sharepoint.com/:b:/g/personal/joona_kareinen_lut_fi/EX2Cg0oipGBMuAWyBqPCGsEBP51TyucXHU8fvmKrj5LhKw?e=5KriFY"} />
        </div >
    )
}

function SVGButton({ path, name, link }) {

    return (
        <button className={`${buttonTheme.base} ${buttonTheme.hover}`}>
            <a href={link} rel="noopener noreferrer" target="_blank" className="flex">
                <svg role="img" viewBox="0 0 24 24" fill="currentColor" className={buttonTheme.icon} xmlns="http://www.w3.org/2000/svg" >
                    <title>{name}</title>
                    <path d={path} />
                </svg >
                <p className="sm:text-base self-center text-xs">{name}</p>
            </a>
        </button>
    )
